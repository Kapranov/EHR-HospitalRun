class Appointment
  ...
  attr_accessor :contact_phone_code, :contact_phone_tel

  scope :today, -> { where(:appointment_datetime.gt => DateTime.now.in_time_zone(Time.zone).beginning_of_day,
                           :appointment_datetime.lt => DateTime.now.in_time_zone(Time.zone).end_of_day) }

  self_select_collection :durations, :itself, :itself
  ...
  def create_calendar
    Calendar.create(calendar_params.merge({appointment_id: id}))
  end
  ...
  def calendar_params
    {
        ...
        start_time: appointment_datetime.try(:to_time),
  ...
  def get_datetimes
    ...
    self.appointment_datetime_date ||= self.appointment_datetime.to_date.to_s(:frontend_date)
    self.appointment_datetime_time ||= "#{'%02d' % self.appointment_datetime.to_time.hour}:#{'%02d' % self.appointment_datetime.to_time.min}"
  end

  def set_datetimes
    self.appointment_datetime = "#{Date.strptime(self.appointment_datetime_date, Date::DATE_FORMATS[:frontend_date]).to_s(:db)} #{self.appointment_datetime_time}".to_time
  end

  def get_phones
    if self.contact_phone.present?
      self.contact_phone_code = self.contact_phone[2..4]
      self.contact_phone_tel = self.contact_phone[5..self.contact_phone.size-1]
    end
  end

  def set_phones
    self.contact_phone = [self.contact_phone_code, self.contact_phone_tel].join
  end
