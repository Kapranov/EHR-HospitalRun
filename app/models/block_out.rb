class BlockOut
  ...
  self_select_collection :time_fors, :itself, :itself
  ...
  def get_datetimes
    ...
    self.block_datetime_date ||= self.block_datetime.to_date.to_s(:frontend_date)
    self.block_datetime_time ||= "#{'%02d' % self.block_datetime.hour}:#{'%02d' % self.block_datetime.min}"
  end

  def set_datetimes
    self.block_datetime = "#{Date.strptime(self.block_datetime_date, Date::DATE_FORMATS[:frontend_date]).to_s(:db)} #{self.block_datetime_time}".to_time
  end
end
