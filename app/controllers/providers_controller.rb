class ProvidersController < ApplicationController
  ...
  skip_before_action :verify_authenticity_token, only: [:save_message]

  layout 'providers'

  def index
    authorize Provider, :provider?
    @appointments = current_user.provider.appointments.today.paginate(page: 1, per: 10)
  ...
  def create
    ...
    respond_to do |format|
      if @provider.save
        format.html { redirect_to providers_path, notice: creation_notification(@provider) }
  ...
  def send_invite_email
    authorize Patient, :create?
    InvitationMailer.send_invitation(params[:name], params[:last_name], params[:code], params[:email]).deliver
    render nothing: true
  end
  ...
  def invite_to_portal_redirect_patients_index
  ...
  def save_message
    ...
    if message.persisted?
      ...
      flash[:notice] = creation_notification(message)
  ...
  def providers
    ...
    render json: if params[:part].present?
                   current_user.provider.find_providers_by(params[:part].downcase.titleize)
                 else
