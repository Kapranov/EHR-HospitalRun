class TablesController < ApplicationController
  ...
  before_action :prepare_model_name, only:   [:records]
  before_action :set_class_name,     except: [:index]
  before_action :set_fields_and_associations, only: [:new, :edit]
  before_action :find_instance,      only:   [:edit, :update, :destroy]
  ...
  def create
    ...
    if instance.persisted?
      flash[:notice] = creation_notification(@class_name)
  ...
  def update
    if @instance.update(class_params)
      flash[:notice] = updation_notification(@class_name)
      redirect_to records_tables_path(model_name: params[:model_name])
    else
      flash[:error] = @instance.errors.full_messages.to_sentence
      redirect_to :back
  ...
  def destroy
    @instance.destroy
    flash[:notice] = deletion_notification(@class_name)
    redirect_to records_tables_path(model_name: params[:model_name])
  end
