class SettingsController < ApplicationController
  ...
  def practice
    ...
    @locations = current_user.main_provider.locations
    @practices = current_user.provider.providers if policy(:practice).show?
  end

  def access_permissions
    authorize Provider, :admin?
    @permissions = current_user.main_provider.permissions
  end
  ...
  def update_permissions
    ...
    flash[:notice] = updation_notification(current_user.provider.permissions.first)
    redirect_to :back
  end
  ...
  def update_schedule
    ...
    if schedule.update(schedule_params)
      flash[:notice] = updation_notification(schedule)
    else
