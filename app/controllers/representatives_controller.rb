class RepresentativesController < ApplicationController
  ...
  before_action :find_representative,      only: [:edit, :update, :show, :destroy, :activate, :reset_password]
  before_action :find_all_representatives, only: [:index, :activate]
    ...
    if params[:representative_policy]
      ...
      if user.persisted?
        ...
        if representative.persisted?
          ...
          flash[:notice] = creation_notification(representative)
  ...
  def update
    if @user.update(user_params)
      if @representative.update(representative_params)
        flash[:notice] = updation_notification(@representative)
        redirect_to representatives_path(patient_id: @patient.id)
      else
        flash[:error] = @representative.errors.full_messages.to_sentence
        redirect_to :back
