class EmailMessagesController < ApplicationController
  ...
  before_action proc { authorize EmailMessage, :show? }
  ...
  def create_main
    ...
    if message.persisted? && (params[:commit] == 'Send' ?  Phimail.send(message) : true)
      # EmailMessagesMailer.send_message(email_message).deliver if params[:commit] == 'Send'
      log 1, 0, patient.id
      flash[:notice] = creation_notification_new(t('model_titles.email_message'))
  ...
  def create_to_practice
    if params[:email_message][:patient_id].present? && params[:practice_id].present?
      patient  = Patient.find(params[:email_message][:patient_id])
      practice = Provider.find(params[:practice_id])
      ...
      if message1.persisted? && message2.persisted? && (params[:commit] == 'Send' ?  Phimail.send(message1) : true) && (params[:commit] == 'Send' ?  Phimail.send(message2) : true)
        ...
        flash[:error] = message1.errors.full_messages.to_sentence
        redirect_to :back
      end
    else
      flash[:error] = 'You should set patient'  if params[:email_message][:patient_id].blank?
      flash[:error] = 'You should set practice' if params[:practice_id].blank?
      redirect_to :back
    end
  end
  ...
  def create_from_patient_to_practice
    if params[:to_id].present?
      ...
      flash[:error] = 'You should set practice'
      redirect_to email_messages_path
    end
  end
  ...
  def find_practices
    ...
    practices = if params[:part].present?
                  current_user.provider.find_providers_by(params[:part])
                else
                  current_user.provider.providers_first(10)
  ...
  def patients
    ...
    render json: if params[:part].present?
                   current_user.provider.find_patients_by(params[:part])
                 else
                   current_user.provider.get_patients
                 end.find_all{ |patient| !patient.user.no_email }
                    .map{ |patient| { full_name: patient.full_name, id: patient.id } }
  end
  ...
  def practices
    ...
    practices = if params[:is_in_practice] == 'true'
                ...
                end.map{ |practice| { full_name: practice.full_name, id: practice.id } }
    render json: practices
  end
  ...
  def patient_practices
    ...
    providers = if params[:part].present?
                  current_user.patient.provider.find_providers_by(params[:part])
                else
                  current_user.patient.provider.providers_first(10)
